...
def _apply_oracle_ins_masks(
    ...
    in_tokens[~(in_tokens.ne(unk_idx))] = 10000000
    out_masks = out_tokens.ne(padding_idx)
    masked_tgt_masks, _, mask_ins_targets = _get_advanced_ins_targets(in_tokens, out_tokens, padding_idx, unk_idx)
    # if not torch.all(masked_tgt_masks.sum().eq(mask_ins_targets.sum())):
    #     print(masked_tgt_masks.sum(), mask_ins_targets.sum())
    in_tokens_tmp = in_tokens.clone()
    int_out = _apply_ins_masks(in_tokens_tmp, None, None, mask_ins_targets, padding_idx, unk_idx, eos_idx)[0] 
    int_mask = ~(int_out.ne(unk_idx))
    # if not torch.all(int_mask.sum(axis=1).eq(mask_ins_targets.sum(axis=1))):
    #     print(mask_ins_targets.sum(axis=1), int_mask.sum(axis=1))
    int_out.masked_scatter_(
        int_mask, out_tokens[masked_tgt_masks]
    )

    int_out[~(int_out.ne(10000000))] = unk_idx
...
def _get_advanced_ins_targets(in_tokens, out_tokens, padding_idx, unk_idx, eos_idx=None):
    ...
    def _get_ins_targets_cuda(in_tokens, out_tokens, padding_idx, unk_idx, eos_idx=None):
        ...
        # This is incorrect -> the modeil should generate the maksed_tgt_tokens from 
        masked_tgt_tokens = out_tokens.masked_fill(masked_tgt_masks, unk_idx)

        # in_tokens_tmp = in_tokens.clone()
        # masked_src_tokens = _apply_ins_masks(in_tokens_tmp, None, None, mask_ins_targets, padding_idx, unk_idx, eos_idx)[0] 
        # masked_src_masks = ~(masked_src_tokens.ne(unk_idx))
        # masked_src_tokens[~(masked_src_tokens.ne(10000000))] = unk_idx
