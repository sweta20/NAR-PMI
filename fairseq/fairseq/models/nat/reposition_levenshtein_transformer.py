...
class RepositionLevenshteinTransformerModel(FairseqNATModel):
    ...
    def forward(
        ...
        # print(self.decoder.prob)
        if random.random() < self.decoder.prob:  # insertion -> reposition & deletion

            # generate training labels for insertion
            ins_tgt_tokens = _apply_oracle_ins_masks(prev_output_tokens, tgt_tokens, self.pad, self.unk, self.eos)

            if ins_tgt_tokens.shape[1] <  prev_output_tokens.shape[1]:
                ins_tgt_tokens = torch.cat((ins_tgt_tokens, torch.cuda.LongTensor(ins_tgt_tokens.shape[0], \
                    prev_output_tokens.shape[1] - ins_tgt_tokens.shape[1]).fill_(self.eos)), axis=1)
            else:
                ins_tgt_tokens = ins_tgt_tokens[:, :prev_output_tokens.shape[1]]

            # masked_tgt_masks = Mask over tgt tokens used for loss fill-in tokens
            # masked_tgt_tokens = input to the fill-in module with tokens to predict substituted
            # mask_ins_target = # of tokens to insert between consecutive tokens
            masked_tgt_masks, masked_tgt_tokens, mask_ins_targets = _get_advanced_ins_targets(
                prev_output_tokens, ins_tgt_tokens, self.pad, self.unk, self.eos
            )
            ...
            use_oracle = random.random()
            if use_oracle < self.decoder.oracle_prob:
                word_predictions = ins_tgt_tokens
            else:
                ...
                word_predictions.masked_scatter_(
                    ~masked_tgt_masks, ins_tgt_tokens[~masked_tgt_masks]
                )
            ...
            if (use_oracle < self.decoder.oracle_prob) and ( self.decoder.not_update_roll_in):
                return {
                        "mask_ins": {
                            "out": mask_ins_out, "tgt": mask_ins_targets,
                            "mask": mask_ins_masks, "ls": 0.01, "factor": 0.0,
                        },
                        "word_ins_ml": {
                            "out": word_ins_out, "tgt": ins_tgt_tokens,
            ...
            # print("reposition & deletion -> insertion")
            ...
            use_oracle = random.random()
            if use_oracle < self.decoder.oracle_prob:
                word_predictions = word_reposition_targets
            else:
                # make online prediction
            ...
            ins_tgt_tokens = _apply_oracle_ins_masks(word_predictions, tgt_tokens, self.pad, self.unk, self.eos)

            if ins_tgt_tokens.shape[1] <  word_predictions.shape[1]:
                ins_tgt_tokens = torch.cat((ins_tgt_tokens, torch.cuda.LongTensor(tgt_tokens.shape[0], \
                    word_predictions.shape[1] - ins_tgt_tokens.shape[1]).fill_(self.eos)), axis=1)
            else:
                ins_tgt_tokens = ins_tgt_tokens[:, :word_predictions.shape[1]]

            # generate training labels for insertion
            masked_tgt_masks, masked_tgt_tokens, mask_ins_targets = _get_advanced_ins_targets(
                word_predictions, ins_tgt_tokens, self.pad, self.unk, self.eos
            )
            mask_ins_targets = mask_ins_targets.clamp(min=0, max=255)  # for safe prediction
            ...
            if (use_oracle < self.decoder.oracle_prob) and ( self.decoder.not_update_roll_in):
                return {
                        ...
                        "word_reposition": {
                            "out": word_reposition_out, "tgt": word_reposition_targets,
                            "mask": word_reposition_masks, "factor": 0.0,
                        }
                    }
