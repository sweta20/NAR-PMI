...
# Path to moses and readability
parent_path=$( cd "$(dirname "${BASH_SOURCE[0]}")" ; pwd -P )
moses_scripts=$parent_path/../scripts
metric_path=$parent_path/../readability
...
if [ $evaluate == True ]; then
	...
	if [[ $use_split == ours-fil ]]; then
		src=data/$subset.src.nograde
		tgt=data/$subset.tgt
		extra_args=" --grade_file data/$subset.grade.tgt --pmi-weights notebooks/data/PMI_nltk.pkl "
