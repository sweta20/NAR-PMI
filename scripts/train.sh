...
run_n=3
tokenize=False
while getopts "i:j:m:u:n:z:s:r:g:a:c:f:p:k:t" opt; do
	case $opt in
		i)
			eid=$OPTARG ;;
		j)
...
if [[ ${model_type} == nat ]]; then
    if [[ $nat_type == lev ]]; then
        architecture=levenshtein_transformer
    else
...
echo "eid:$eid, did:$did, model_type=$model_type, nat_type=${nat_type}, data_split=${use_split}, noise_type=${noise_type}, architecture=${architecture}, arguments=${model_args}" > $exp_dir/config
cat $exp_dir/config
...
for seed in $(seq 1 $run_n); do
    ...
    if [ ! -f $model_dir/checkpoint_best.pt ]; then
        if [ $model_type == nat ]; then
            ...
            # apply-bert-init only applies initialization as required by the bert model
        else
        	CUDA_VISIBLE_DEVICES=0,1 fairseq-train \
        	    ...
        	    --max-tokens 4096 --save-dir ${model_dir}  --keep-last-epochs 20 \
                --max-epoch 30 --patience 8 --best-checkpoint-metric ppl ${model_args} > ${model_dir}/log  
        fi;
    fi;
done;
